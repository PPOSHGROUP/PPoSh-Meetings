...
Describe -Name 'Presentation Test' -Fixture {
  ...
  Context -Name 'Presentation' -Fixture {
    ...
    It -name 'Should have the correct PowerPoint Presentation Open' -test {(Get-Process -Name POWERPNT  -ErrorAction SilentlyContinue).MainWindowTitle| Should Be 'PPOSH_SU.pptx - PowerPoint'}
  }
  Context -Name 'Windows environment' -Fixture {
    ...
    It -name 'Teams should be closed' -test {(Get-Process -Name Teams.exe -ErrorAction SilentlyContinue).Count | Should BE 0}
    It -name 'Screenpresso should be closed' -test {(Get-Process -Name Screenpresso.exe -ErrorAction SilentlyContinue).Count | Should BE 0}
  }
  Context -Name 'Windows environment' -Fixture {
  ...
  $Files = 10
    It -name 'Should exist file PPOSH_Function.pptx' -test {Test-path -Path 'C:\Users\tdabrowski\Desktop\PPoSh\04\PPOSH_SU.pptx' | Should Be True }
    It -name "Should exist $Files files" -test {(Get-ChildItem -Path 'C:\Users\tdabrowski\Desktop\PPoSh\04' -Recurse).Count | Should Be $Files}
  }
}