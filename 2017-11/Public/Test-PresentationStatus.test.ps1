...
Describe -Name 'Presentation Test' -Fixture {
  ...
  Context -Name 'Presentation' -Fixture {
    ...
    It -name 'Should have the correct PowerPoint Presentation Open' -test {(Get-Process -Name POWERPNT  -ErrorAction SilentlyContinue).MainWindowTitle| Should Be 'PPOSH_EmplMaintenance.pptx - PowerPoint'}
  }
  ...
  $Files = 4
    It -name 'Should exist file PPOSH_Function.pptx' -test {Test-path -Path 'C:\Users\tdabrowski\Desktop\PPoSh\07\PPOSH_EmplMaintenance.pptx' | Should Be True }
    It -name "Should exist $Files files" -test {(Get-ChildItem -Path 'C:\Users\tdabrowski\Desktop\PPoSh\07' -Recurse).Count | Should Be $Files}
  }
}